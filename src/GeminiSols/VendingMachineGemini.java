...
public class VendingMachineGemini implements VendingMachine {
    ...
    public List<Product> getProductStock() {
        // Return an unmodifiable copy of the product stock
        return Collections.unmodifiableList(productStock);
    }
    ...
    public void removeProduct(String name) throws ProductNotFoundException {
        boolean removed = productStock.removeIf(product -> product.getName().equals(name));
        if (!removed) {
            throw new ProductNotFoundException("Product with name " + name + " not found");
        }
    }
    ...
    public double getBackMoney() {
        double moneyToReturn = activeBalance;
        activeBalance = 0.0;
            ...
            if (product.getName().equals(name)) {
                if (product.getStock() > 0) {
                    if (activeBalance >= product.getPrice()) {
                        ...
                        change = activeBalance;
                        activeBalance = 0.0;
                        return change;
                    } else {
                        throw new InsufficientMoneyException("Insufficient funds for purchase");
                    }
                } else {
